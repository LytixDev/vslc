...
static void gen_struct(Compiler *compiler, Symbol *sym)
{
    ...
    for (u32 i = 0; i < t->members_len; i++) {
        ...
        if (m_t->kind == TYPE_ARRAY) {
            ...
            fprintf(f, "\t%s %s%s[%d];\n", m_t->generated_by_name.str, ptr_modifier, m->name.str, type_as_array->elements);
        } else {
...
static void gen_expr(Compiler *compiler, SymbolTable *symt_local, AstExpr *head)
{
    ...
    case EXPR_BINARY: {
        ...
        switch (expr->op) {
            ...
            case TOKEN_LSHIFT: fprintf(f, " << "); break;
            case TOKEN_RSHIFT: fprintf(f, " >> "); break;
            case TOKEN_DOT: fprintf(f, "."); break;
